...
export const BrandingTab: React.FC<BrandingTabProps> = ({
  ...
  const [customPathInput, setCustomPathInput] = useState('vipulanantha-college-logo.png');
  const [isApplyingPath, setIsApplyingPath] = useState(false);

  const handleApplyLogoPath = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      const path = customPathInput.trim().replace(/^school-assets\//, '');
      const publicUrl = getOfficialSupabaseLogoUrl(path);
      await applySchoolLogoUrl(publicUrl, path);

      const updated = {
        ...formData,
        logoUrl: publicUrl,
        loginPageLogoUrl: publicUrl,
      };
      setFormData(updated);
      await onSave(updated);
      setNotification({
        type: 'success',
        message: `Successfully configured school-assets/${path} as official school logo!`,
      });
      setTimeout(() => setNotification(null), 5000);
    } catch (err: any) {
      setNotification({
        type: 'error',
        message: err.message || 'Failed to apply logo path.',
      });
    } finally {
            ...
            <div className="w-full space-y-2">
              ...
              <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4 space-y-3">
                <div className="flex items-center space-x-2 text-xs font-bold text-slate-800">
                  ...
                  <span>Supabase Storage Bucket: <code className="font-mono text-purple-950 font-extrabold">school-assets</code></span>
                </div>
                <p className="text-[11px] text-slate-600">
                  Enter the file path inside your public <code className="font-mono text-purple-900">school-assets</code> bucket:
                </p>
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    placeholder="vipulanantha-college-logo.png"
                    className="flex-1 bg-white border border-slate-300 rounded-xl px-3 py-2 text-xs font-mono text-slate-900 focus:outline-none focus:ring-2 focus:ring-purple-900"
                  />
                  <button
                    type="button"
                    onClick={handleApplyLogoPath}
                    disabled={isApplyingPath || !customPathInput.trim()}
                    className="px-4 py-2 bg-purple-900 hover:bg-purple-950 text-white text-xs font-bold rounded-xl cursor-pointer disabled:opacity-50 shrink-0 flex items-center space-x-1"
                  >
                    {isApplyingPath ? (
              ...
              >
                ...
                <span>Reset to Default School Logo</span>
              </button>
            </div>
          )}
        </div>
