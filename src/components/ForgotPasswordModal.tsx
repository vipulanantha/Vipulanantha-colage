      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-2xl border border-purple-100 overflow-hidden">
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="flex items-center space-x-3 mb-6">
            ...
            <div>
              <h2 id="forgot-password-title" className="text-xl font-bold text-slate-900 font-cinzel">
                Password Recovery
              </h2>
              <p className="text-xs text-slate-500">Vipulanantha College Student & Staff Portal</p>
            </div>
          </div>
          ...
          {step === 1 && (
            <form onSubmit={handleSendCode} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 text-sm bg-slate-50 border border-slate-300 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-purple-600 transition-all"
                  />
                </div>
                <p className="mt-1 text-[11px] text-slate-500">
                  A 6-digit verification security code will be dispatched to your linked mobile or email.
                </p>
              </div>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-2.5 px-4 rounded-xl bg-purple-900 hover:bg-purple-950 text-white text-sm font-semibold shadow-md hover:shadow-lg transition-all flex items-center justify-center space-x-2 cursor-pointer disabled:opacity-70"
              >
                {isLoading ? (
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4">
                ...
                <div className="flex justify-center space-x-2">
                  {otp.map((digit, index) => (
                    <input
                      ...
                      className="w-10 h-12 text-center text-lg font-bold bg-slate-50 border border-slate-300 rounded-lg focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-purple-600"
                    />
                  ))}
                </div>
              ...
              <div className="flex space-x-3">
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="flex-1 py-2.5 px-4 rounded-xl bg-purple-900 hover:bg-purple-950 text-white text-sm font-semibold shadow-md transition-all flex items-center justify-center space-x-1 cursor-pointer disabled:opacity-70"
                >
                  {isLoading ? (
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  ) : (
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 text-sm bg-slate-50 border border-slate-300 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-600 focus:border-purple-600"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-2.5 px-4 rounded-xl bg-gradient-to-r from-purple-900 to-blue-900 hover:from-purple-950 hover:to-blue-950 text-white text-sm font-semibold shadow-md transition-all flex items-center justify-center space-x-2 cursor-pointer disabled:opacity-70"
              >
                {isLoading ? (
                  <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                ) : (
